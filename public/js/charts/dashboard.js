(function (jQuery) {
  ...
  if (document.querySelectorAll('#d-main').length && window.__DASHBOARD_CHART__) {
    const chartData = window.__DASHBOARD_CHART__;

    const options = {
      series: [{
        name: 'Sales',
        data: chartData.sales  // ← sales (subtotal per month)
      }, {
        name: 'Cost',
        data: chartData.costs  // ← costs (20% of sales per month)
      }],
      chart: {
      ...
      legend: { show: false },
      xaxis: {
        categories: chartData.months, // ← dynamic months (e.g. ["Mar", "Apr", "May"])
        labels: {
          minHeight: 22,
          maxHeight: 22,
          show: true,
    ...
    new ApexCharts(document.querySelector("#d-main"), options).render();
  }
